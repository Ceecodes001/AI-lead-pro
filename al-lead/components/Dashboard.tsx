...
const GlassCard: React.FC<{
  ...
  <div
    className={`bg-card-bg border border-white/10 rounded-2xl shadow-lg backdrop-blur-lg p-4 sm:p-6 ${className}`}
  >
    {children}
  </div>
);
...
const CampaignCard: React.FC<{
  campaign: Campaign & { calculatedOpenRate: number };
}> = ({ campaign }) => (
...
const Dashboard: React.FC<{ leads: Lead[]; campaigns: Campaign[] }> = ({
  ...
  const campaignStats = useMemo(() => {
    return campaigns.map((campaign) => {
      const campaignLeads = leads.filter((l) =>
        campaign.leadIds?.includes(l.id)
      );
      ...
      for (const lead of campaignLeads) {
        const sentEvent = lead.activity?.find(
          (a) => a.type === "Email Sent" && a.campaignId === campaign.id
        );
        if (sentEvent) {
          sentCount++;
          const openedEvent = lead.activity?.find(
            (a) => a.type === "Email Opened" && a.campaignId === campaign.id
          );
          if (openedEvent) {
            openCount++;
          }
  ...
  useEffect(() => {
    const generateSuggestion = async () => {
      ...
      const totalOpenRate = activeCampaigns.reduce(
        (sum, c) => sum + c.calculatedOpenRate,
        0
      );
      const avgOpenRate =
        activeCampaigns.length > 0
          ? (totalOpenRate / activeCampaigns.length).toFixed(2)
          : 0;
    ...
    if (leads.length > 0 || campaigns.length > 0) {
      ...
      setAiSuggestion(
        "Add some leads or create a campaign to get started with AI suggestions."
      );
      setIsLoadingSuggestion(false);
    }
  ...
  const contactedLeadsCount = useMemo(
    () =>
      leads.filter((l) => l.status === "Contacted" || l.status === "Qualified")
        .length,
    [leads]
  );
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
      ...
      <GlassCard className="lg:col-span-1 text-center">
        <h3 className="font-semibold text-text-muted mb-1 text-sm md:text-base">
          Total Leads
        </h3>
      ...
      <GlassCard className="lg:col-span-1 text-center">
        ...
        <p className="text-3xl md:text-4xl font-bold">{contactedLeadsCount}</p>
      </GlassCard>
      <GlassCard className="lg:col-span-1 text-center">
        <h3 className="font-semibold text-text-muted mb-1 text-sm md:text-base">
          Qualified
      ...
      {/* AI Suggestion */}
      <GlassCard className="sm:col-span-2 lg:col-span-1 flex flex-col justify-center items-center text-center bg-accent/10 border-accent/30">
        <svg
      ...
      <GlassCard className="sm:col-span-2 lg:col-span-2">
        <h3 className="text-lg md:text-xl font-poppins font-semibold mb-4">
