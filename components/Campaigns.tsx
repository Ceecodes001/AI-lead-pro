...
import {
  ...
  generateInitialCampaignEmail,
  suggestLeadsForCampaign,
...
const GlassCard: React.FC<{
  ...
  <div
    className={`bg-card-bg border border-white/10 rounded-2xl shadow-lg backdrop-blur-lg p-6 ${className}`}
  >
    {children}
  </div>
);
...
const EmailBuilder: React.FC<{
  ...
  const [draggedType, setDraggedType] = useState<EmailComponentType | null>(
    null
  );
  ...
  const handleDrop = (e: React.DragEvent, index?: number) => {
      ...
      switch (draggedType) {
        case "text":
          newItem = {
            id: createId(),
            type: "text",
            content: "This is a new text block. Click to edit.",
          };
          break;
        case "image":
          newItem = {
    ...
    const newBody = draggedItem
      ? body.filter((item) => item.id !== draggedItem.id)
      : [...body];
  ...
  const handleUpdate = (id: string, newProps: Partial<EmailComponent>) => {
    setBody(
      body.map((item) =>
        item.id === id ? ({ ...item, ...newProps } as EmailComponent) : item
      )
    );
  };
        ...
        return (
          <div
            ...
            dangerouslySetInnerHTML={{
              __html: textItem.content.replace(/\n/g, "<br />"),
            }}
          />
        );
      case "image":
      ...
      case "button":
        ...
        return (
          <a
            ...
            className="inline-block bg-primary text-white font-bold py-2 px-6 rounded-lg no-underline"
          >
            {btnItem.text}
          </a>
        );
  ...
  const openEditModal = (item: EmailComponent) => {
    ...
    if (item.type === "text") {
      ...
      const newText = prompt(
        "Edit button text:",
        (item as ButtonComponent).text
      );
      const newHref = prompt(
        "Edit button URL:",
        (item as ButtonComponent).href
      );
      ...
      if (newSrc !== null)
        handleUpdate(item.id, { src: newSrc } as Partial<ImageComponent>);
    } else if (item.type === "spacer") {
      const newHeight = prompt(
        "Edit spacer height (px):",
        (item as SpacerComponent).height.toString()
      );
      if (newHeight !== null)
        handleUpdate(item.id, {
          height: parseInt(newHeight, 10),
        } as Partial<SpacerComponent>);
    }
  };
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-1 gap-2">
          {sidebarItems.map(({ type, label }) => (
            <div
              key={type}
              draggable
              onDragStart={(e) => handleDragStart(e, null, type)}
      ...
      {/* Canvas */}
      <div
        className="flex-1 bg-black/30 rounded-lg p-4 min-h-[400px]"
        onDragOver={handleDragOver}
        onDrop={(e) => handleDrop(e)}
        onDragLeave={handleCanvasDragLeave}
      >
        <div
          id="indicator-dropzone-start"
          className="h-1 bg-primary rounded-full scale-x-0 origin-center drop-indicator transition-transform duration-200 ease-in-out pointer-events-none"
        ></div>
        {body.length === 0 ? (
          <div
            className="flex items-center justify-center h-full text-text-muted"
            onDragEnter={(e) =>
              handleDragEnterItem(e, "indicator-dropzone-start")
            }
          >
          ...
          body.map((item, index) => (
            <div
              ...
              onDragEnter={(e) => handleDragEnterItem(e, `indicator-${index}`)}
            >
              <div className="relative group border border-transparent hover:border-primary/50 rounded-md p-1">
                <div className="absolute top-1 right-1 z-10 opacity-0 group-hover:opacity-100 transition-opacity bg-background/80 rounded-md flex items-center">
                  ...
                  <div
                    ...
                    className="p-1.5 cursor-grab text-text-muted hover:text-white"
                  >
                    <svg
                      className="w-4 h-4"
                      fill="none"
...
const EmailTemplatesModal: React.FC<{
  ...
  const [aiPrompt, setAiPrompt] = useState(
    "A welcome email for a new subscriber to a tech newsletter"
  );
  ...
  const templates: { [key: string]: EmailComponent[] } = {
    "Welcome Email": [
      { id: createId(), type: "text", content: "## Welcome Aboard!" },
      {
        ...
        type: "text",
        content:
          "We are so excited to have you. Here is what you can expect from us...",
      },
    ...
    "Product Launch": [
      ...
      },
      {
        ...
        type: "text",
        content:
          "Discover the amazing features of our latest innovation designed to solve your problems.",
      },
      {
        id: createId(),
        type: "button",
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <GlassCard className="w-[95vw] max-w-3xl">
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            onClick={onClose}
            className="p-2 rounded-full hover:bg-white/10 text-2xl leading-none"
          >
            &times;
          </button>
        </div>
        <div className="mb-8">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            <button
              ...
              className="py-2 px-4 bg-accent hover:bg-accent/80 rounded-lg transition-colors shadow-glow-accent disabled:opacity-50 flex items-center justify-center"
            >
              {isGenerating && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {Object.entries(templates).map(([name, structure]) => (
              <div
                ...
                className="border border-white/10 rounded-lg p-4 hover:bg-primary/20 hover:border-primary cursor-pointer transition-all"
              >
                <h4 className="font-bold">{name}</h4>
                <p className="text-sm text-text-muted mt-1">
                  {structure.map((s) => s.type).join(", ")}
                </p>
              </div>
            ))}
          </div>
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[60] p-4">
      <GlassCard className="w-[95vw] max-w-md">
        ...
        <p className="text-text-muted mb-4">
          Set a date and time to automatically activate the campaign "
          {campaignName}".
        </p>
        ...
        <div className="mt-6 flex justify-end gap-4">
          ...
          <button
            ...
            className="py-2 px-4 bg-accent hover:bg-accent/80 rounded-lg transition-colors shadow-glow-accent"
          >
            Confirm Schedule
          </button>
        </div>
...
const CampaignEditorModal: React.FC<{
  ...
  onSave: (
    campaignData: Omit<
      Campaign,
      "id" | "startDate" | "openRate" | "clickRate" | "leads" | "leadIds"
    >,
    leadIds: string[],
    campaignId?: string
  ) => void;
  ...
  const handleAddVariant = () => {
    ...
    setVariants([
      ...variants,
      { id: createId(), name: `Version ${nextChar}`, subject: "", body: [] },
    ]);
  };

  const handleContentChange = (
    field: "subject" | "body",
    value: string | EmailComponent[]
  ) => {
      ...
      setVariants((current) => {
        ...
        if (newVariants[editingTarget.index]) {
          const updatedVariant = {
            ...newVariants[editingTarget.index],
            [field]: value,
          };
      ...
      setFollowUps((current) => {
        ...
        if (newFollowUps[editingTarget.index]) {
          const updatedFollowUp = {
            ...newFollowUps[editingTarget.index],
            [field]: value,
          };
  ...
  const handleFollowUpRuleChange = (
    index: number,
    field: "trigger" | "delayDays",
    value: string | number
  ) => {
    setFollowUps((current) => {
      const newFollowUps = [...current];
      const updatedFollowUp = {
        ...newFollowUps[index],
        [field]: field === "delayDays" ? Number(value) || 0 : value,
      };
  ...
  const handleRemoveFollowUp = (indexToRemove: number) => {
    if (
      window.confirm("Are you sure you want to remove this follow-up email?")
    ) {
  ...
  const handleFindEmails = async () => {
    ...
    const emailPromises = leadsWithoutEmail.map((lead) =>
      findLeadEmail(lead.name, lead.company).then((email) => ({
        leadId: lead.id,
        email,
      }))
    );
  ...
  const handleSubmit = () => {
    ...
    if (status !== "Draft" && selectedLeadIds.size === 0) {
      alert(
        "Please select at least one lead for an active or paused campaign."
      );
      return;
    }
  ...
  const handleSchedule = (dateTime: string) => {
    ...
    if (selectedLeadIds.size === 0) {
      alert("Please select at least one lead to schedule a campaign.");
      return;
    }
              ...
              <div>
                ...
                <div className="flex flex-col sm:flex-row gap-2 items-center">
                  <input
                    ...
                    placeholder="e.g., Q3 Web Design Outreach"
              ...
              <div>
                <div className="flex justify-between items-center mb-2 flex-wrap gap-2">
                  ...
                  >
                    ...
                    <span>
                      {isSuggestingLeads ? "Thinking..." : "AI Suggest"}
                    </span>
                  </button>
                </div>
                <div className="flex flex-col sm:flex-row justify-between items-center mb-2 flex-wrap gap-2">
                  <input
                    ...
                    className="w-full sm:w-auto bg-card-bg border border-white/10 rounded-lg py-2 px-4 focus:outline-none focus:ring-2 focus:ring-primary flex-grow"
                  />
                  <button
                    onClick={handleFindEmails}
                    disabled={isFindingEmails || selectedLeadIds.size === 0}
                    className="w-full sm:w-auto flex items-center justify-center gap-2 py-2 px-4 bg-primary/70 hover:bg-primary/100 rounded-lg transition-colors text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isFindingEmails && (
                      <div className="animate-spin rounded-full h-4 w-4 border-t-2 border-b-2 border-white"></div>
                    )}
                ...
                <p className="text-sm text-text-muted mb-2">
                  {selectedLeadIds.size} / {leads.length} leads selected
                </p>
                  ...
                  {filteredLeads.map((lead) => (
                    ...
                    >
                      <input
                        ...
                        className="h-5 w-5 rounded bg-card-bg border-white/30 text-primary focus:ring-primary focus:ring-offset-0 accent-primary"
                      />
                      <div>
                ...
                <div className="space-y-3">
                  ...
                  {variants.map((variant, index) => (
                    <button
                      key={variant.id}
                      onClick={() =>
                        setEditingTarget({ type: "variant", index })
                      }
                      className={`w-full text-left p-3 rounded-lg transition-colors text-sm border-l-4 ${
                        editingTarget.type === "variant" &&
                        editingTarget.index === index
                          ? "bg-primary/20 border-primary"
                          : "bg-card-bg border-transparent hover:bg-white/10"
                      }`}
                    >
                      <p className="font-medium">{variant.name}</p>
                      <p className="text-xs text-text-muted truncate">
                        {variant.subject || "No subject set"}
                      </p>
                    </button>
                  ))}
                    ...
                    <GlassCard key={followUp.id} className="!p-3">
                      <div className="flex justify-between items-center mb-2">
                        ...
                        <button
                          ...
                          className="p-1 rounded-full hover:bg-red-500/20 text-red-400 text-lg leading-none"
                        >
                          &times;
                        </button>
                      </div>
                      ...
                      <button
                        onClick={() =>
                          setEditingTarget({ type: "followup", index })
                        }
                        className={`mt-2 w-full text-left p-2 rounded-md transition-colors text-sm border-l-4 ${
                          editingTarget.type === "followup" &&
                          editingTarget.index === index
                            ? "bg-primary/20 border-primary"
                            : "bg-card-bg border-transparent hover:bg-white/10"
                        }`}
                      >
                        ...
                        <p className="text-xs text-text-muted truncate">
                          {followUp.subject || "No subject set"}
                        </p>
                      </button>
                    </GlassCard>
                  ))}
                  <button
                    ...
                    className="w-full py-2 text-sm text-primary hover:bg-primary/20 rounded-lg border-2 border-dashed border-primary/30 hover:border-primary/60 transition-colors"
                  >
                    + Add Follow-up Step
                  </button>
                </div>
            ...
            <div className="lg:col-span-2">
              <div className="flex justify-between items-center mb-4 flex-wrap gap-2">
                ...
                <div className="flex items-center gap-2">
                  {isVariant && variants.length < 3 && (
                    <button
                      onClick={handleAddVariant}
                      className="py-2 px-3 text-sm font-medium text-text-muted hover:bg-white/5 rounded-lg"
                    >
                      Add A/B Variant
                    </button>
                  )}
                  <button
                    ...
                    className="py-2 px-4 bg-primary/20 text-primary hover:bg-primary/40 rounded-lg text-sm transition-colors"
                  >
                    Load Template
                  </button>
                </div>
              ...
              <EmailBuilder
                ...
                setBody={(newBody) => handleContentChange("body", newBody)}
              />
            </div>
          </div>
          ...
          <div className="mt-6 flex flex-col sm:flex-row justify-between items-center flex-shrink-0 gap-4">
            {isVariant ? (
              <button
                ...
                disabled={isGenerating || variants.length < 2}
                className="w-full sm:w-auto flex items-center justify-center gap-2 py-2 px-4 bg-accent/80 hover:bg-accent rounded-lg transition-colors shadow-glow-accent text-sm disabled:opacity-50"
                title={
                  variants.length < 2
                    ? "Add at least one more variant to generate A/B test suggestions."
                    : "Suggest A/B Variations"
                }
              >
                {isGenerating && (
                  <div className="animate-spin rounded-full h-4 w-4 border-t-2 border-b-2 border-white"></div>
            ...
            <div>
              ...
              <button
                ...
                className="py-2 px-4 bg-accent/80 hover:bg-accent rounded-lg transition-colors shadow-glow-accent ml-4"
              >
                Schedule...
              </button>
              <button
                ...
                className="py-2 px-4 bg-primary hover:bg-primary/80 rounded-lg transition-colors shadow-glow-primary ml-4"
              >
    ...
    <GlassCard className="flex flex-col justify-between h-full">
      <div>
        <div className="flex justify-between items-start mb-4">
          ...
          <span
            className={`text-xs font-bold py-1 px-3 rounded-full whitespace-nowrap ${getStatusClasses(
              campaign.status
            )}`}
          >
            {campaign.status}
