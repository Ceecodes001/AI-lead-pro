...
const GlassCard: React.FC<{
  ...
  <div
    className={`bg-card-bg border border-white/10 z-0 rounded-2xl shadow-lg backdrop-blur-lg p-6 ${className}`}
  >
    {children}
  </div>
);
...
const ScoreDisplay: React.FC<{ score: number }> = ({ score }) => {
  ...
  return (
    <div
      className={`w-10 h-10 flex items-center justify-center rounded-full border-2 ${scoreColor} bg-white/5 font-bold text-base`}
    >
      {score}
    </div>
  );
...
const LeadFormModal: React.FC<{
  ...
  useEffect(() => {
    if (isOpen) {
      setFormData({
        ...
        potential: initialData?.potential || 0,
  ...
  const handleChange = (
    ...
    setFormData((prev) => ({
      ...prev,
      [name]: name === "potential" ? Number(value) : value,
    }));
  };
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input
                ...
                className="bg-card-bg border border-white/10 rounded-lg py-2 px-4 focus:outline-none focus:ring-2 focus:ring-primary"
                required
              />
              <input
                type="text"
            ...
            <div className="space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-2 mb-2 min-h-[2.5rem]">
                {formData.tags.map((tag, index) => (
                  <div
                    key={index}
                    className="flex items-center bg-primary/30 text-primary-light rounded-full pl-3 pr-2 py-1 text-sm font-medium"
                  >
                    <span>{tag}</span>
                    <button
                      type="button"
                      onClick={() => handleRemoveTag(tag)}
                      className="ml-2 text-lg leading-none hover:text-white transition-colors"
                    >
                      &times;
                    </button>
                  </div>
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className="py-2 px-4 bg-primary/50 hover:bg-primary/80 rounded-lg transition-colors text-sm"
                >
                  Add
                </button>
              </div>
      ...
      <GlassCard className="w-[95vw] max-w-lg">
        ...
        <div className="mt-6 flex justify-end gap-4">
          ...
          <button
            type="button"
            onClick={() => onAdd(lead)}
            className="py-2 px-4 bg-primary hover:bg-primary/80 rounded-lg transition-colors shadow-glow-primary"
          >
            Add this Lead
          </button>
        </div>
...
const FindLeadsModal: React.FC<{
  ...
  const groupedLeads = useMemo(() => {
    ...
    const groups: Record<
      string,
      { lead: PotentialLead; originalIndex: number }[]
    > = {
      "Email & Phone": [],
      "Email Only": [],
      "Phone Only": [],
    ...
    const nonEmptyGroups = Object.fromEntries(
      Object.entries(groups).filter(
        ([, leadsInGroup]) => leadsInGroup.length > 0
      )
    );
  ...
  const handleAddSelected = () => {
    const leadsToAdd = foundLeads.filter((_, index) =>
  ...
  const allSelected =
    selectedIndices.size === foundLeads.length && foundLeads.length > 0;
        ...
        <GlassCard className="w-[95vw] max-w-3xl bg-gradient-to-br from-background to-[#1a1f36] border border-primary/30">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-2 mb-4">
            <input
              ...
              className="flex-grow w-full bg-card-bg border border-white/10 rounded-lg py-2 px-4 focus:outline-none focus:ring-2 focus:ring-primary"
              placeholder="e.g., local coffee shops with no website"
            />
            <input
              type="number"
              value={count}
              onChange={(e) => setCount(Number(e.target.value))}
              className="bg-card-bg border border-white/10 rounded-lg py-2 px-4 w-20 focus:outline-none focus:ring-2 focus:ring-primary"
            />
            <button
              onClick={handleSearch}
              disabled={isLoading}
              className="py-2 px-4 w-full sm:w-auto bg-accent hover:bg-accent/80 rounded-lg transition-colors shadow-glow-accent disabled:opacity-50 flex items-center justify-center"
            >
              {isLoading ? (
                ...
                <div className="px-4 pb-4 pt-2">
                  {isSummarizing ? (
                    ...
                    <div className="space-y-4">
                      ...
                      <p className="text-sm text-text-muted italic text-center px-2">
                        {summary.overview}
                      </p>
                      <div>
                        <h4 className="font-semibold text-sm text-text-light mb-1">
                          Promising Prospects:
                        </h4>
                        <p className="text-sm text-text-light whitespace-pre-wrap">
                          {summary.promisingProspects}
                        </p>
                      </div>
                    </div>
                  ) : (
                    <p className="text-sm text-text-muted">
                      Could not generate a summary for these leads.
                    </p>
                  )}
                </div>
              </div>
            ...
            <div className="flex justify-between items-center mb-2 px-1">
              <label className="flex items-center gap-2 text-sm text-text-muted cursor-pointer">
                <input
                  ...
                  className="h-4 w-4 rounded bg-card-bg border-white/30 text-primary focus:ring-primary accent-primary"
                />
                Select All
              </label>
              <p className="text-sm text-text-muted">
                {selectedIndices.size} / {foundLeads.length} selected
              </p>
            </div>
          )}
          ...
          <div className="h-64 overflow-y-auto bg-black/20 rounded-lg p-2 space-y-2">
            ...
            {!isLoading &&
              ...
              Object.entries(groupedLeads).map(([groupName, leadsInGroup]) => (
                <div key={groupName}>
                  <h4 className="text-xs font-bold uppercase text-text-muted px-2 pt-2 pb-1">
                    {groupName} ({leadsInGroup.length})
                  </h4>
                  {leadsInGroup.map(({ lead, originalIndex }) => (
                    <div
                      key={originalIndex}
                      className="p-3 bg-card-bg/80 rounded-lg border border-white/10 flex items-center justify-between gap-4 transition-colors hover:border-primary/50"
                    >
          ...
          <div className="mt-6 flex justify-end">
            ...
            <button
              ...
              className="py-2 px-4 bg-primary hover:bg-primary/80 rounded-lg transition-colors shadow-glow-primary ml-4 disabled:opacity-50"
            >
        ...
        {isSent ? (
          <div className="flex flex-col items-center justify-center h-64 text-center">
            ...
            <p className="text-text-muted mt-2">
              Your default email app is ready. The lead has been marked as
              'Contacted'.
            </p>
          </div>
        ) : isLoading ? (
...
const Leads: React.FC<LeadsProps> = ({
  ...
  const handleSaveLead = useCallback(
    async (leadData: Omit<Lead, "lastContacted" | "id"> & { id?: string }) => {
      handleCloseForm();
  ...
  const handleDeleteLead = (id: string) => {
    if (
      window.confirm(
        "Are you sure you want to delete this lead? This will also remove them from any associated campaigns."
      )
    ) {
  ...
  const handleAddPotentialLeads = (
    newLeadsData: Omit<Lead, "id" | "lastContacted">[]
  ) => {
      ...
      <GlassCard className="overflow-x-auto">
        {leads.length > 0 ? (
            ...
            <tbody>
              {leads.map((lead) => (
                ...
                >
                  ...
                  <td className="p-4">
                    <div className="flex flex-wrap gap-1">
                      {lead.tags?.map((tag, index) => (
                        <span
                          key={index}
                          className={`text-xs font-semibold whitespace-nowrap py-1 px-2 rounded-full ${getTagColor(
                            index
                          )}`}
                        >
                          {tag}
                  ...
                  <td className="p-4">
                    <span
                      className={`text-xs font-bold py-1 px-2 rounded-full ${getStatusClass(
                        lead.status
                      )}`}
                    >
                      {lead.status}
          ...
          <div className="text-center py-16">
            ...
            <p className="text-text-muted">
              Add your first lead manually or use the AI to find new ones.
            </p>
          </div>
        )}
      </GlassCard>
