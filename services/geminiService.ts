...
export const getAIInsight = async (prompt: string): Promise<string> => {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        systemInstruction:
          "You are a concise, confident, and intelligent sales and marketing assistant. Provide actionable insights and recommendations based on the data provided. Your tone should be professional and motivating.",
      },
    });
...
export const summarizePotentialLeads = async (
  leads: PotentialLead[]
...
export const findLeadEmail = async (
  leadName: string,
  companyName: string
): Promise<string | null> => {
  try {
    ...
    console.error(
      `Error finding email for ${leadName} at ${companyName}:`,
      error
    );
    return null;
  }
          ...
          items: {
            ...
            properties: {
              ...
              body: {
                type: Type.STRING,
                description:
                  "The email body content. Use placeholders like {{name}} where appropriate.",
              },
            },
...
export const generateDirectEmail = async (
  lead: Lead
): Promise<Email | null> => {
  try {
    const ai = getGenAI();
...
export const generateEmailVariations = async (
  baseSubject: string,
  baseBody: EmailComponent[]
): Promise<{ subject: string; body: EmailComponent[] }[] | null> => {
  try {
    const ai = getGenAI();
    if (!ai) {
...
export const generateEmailTemplate = async (
  ...
  try {
    ...
    return template.map((component: any) => ({
      ...component,
...
export const generateInitialCampaignEmail = async (
  campaignName: string
): Promise<{ subject: string; body: EmailComponent[] } | null> => {
  try {
    const ai = getGenAI();
...
export const suggestLeadsForCampaign = async (
  campaignName: string,
  allLeads: Lead[]
): Promise<string[] | null> => {
  try {
